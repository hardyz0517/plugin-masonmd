...
import type { MasonToolbarIcon } from "../plugins";
...
import masonLocale from "bytemd/locales/zh_Hans.json";
...
import type { MasonTableCell } from "../editor/table-source-model";
import {
...
} from "../editor/mason-markdown-editor-adapter";
import {
...
type MasonToolbarButton = {
  title: string;
  icon: MasonToolbarIcon;
...
const markdownRuntime = createMarkdownRuntime("mason-v1");
...
const createTableCells = (rows: number, columns: number): TableCell[][] =>
  createMasonTableCells(rows, columns);
...
const AUTOSAVE_HISTORY_STORAGE_KEY = "mason-markdown:autosave-history:v1";
...
const readAutosaveRecordsFromStorage = (storageKey: string): AutosaveRecord[] | undefined => {
  try {
...
const readAutosaveRecords = () =>
  readAutosaveRecordsFromStorage(AUTOSAVE_HISTORY_STORAGE_KEY) ?? [];
...
const confirmTableDialog = () => {
  ...
  ctx.appendBlock(serializeMasonTable(tableCells.value));
  tableDialogOpen.value = false;
  focusEditor();
};
...
const getToolbarButtonView = (button: MasonToolbarButton) => {
...
const isToolbarButtonActive = (button: MasonToolbarButton) => {
  const view = getToolbarButtonView(button);
  return view !== undefined && editorView.value === view;
};

const getToolbarButtonTitle = (button: MasonToolbarButton) => {
  const view = getToolbarButtonView(button);
...
const toolbarLeftGroups: MasonToolbarButton[][] = [
  [
    {
      title: "提升一级",
      icon: "headingUp",
...
const toolbarRightGroups: MasonToolbarButton[][] = [
  [
    {
      title: "仅编辑",
      icon: "writeOnly",
...
<template>
  <section
    ...
    :class="{ 'mason-editor-wrapper-fullscreen': isEditorFullscreen }"
  >
    <div class="mason-editor-toolbar">
      <div class="mason-toolbar-side">
        <span
          v-for="(group, groupIndex) in toolbarLeftGroups"
          :key="`left-${groupIndex}`"
          class="mason-toolbar-group"
        >
          <button
            v-for="button in group"
            :key="button.title"
            type="button"
            class="mason-toolbar-tool"
            :class="{
          ...
          >
            <span
              ...
              v-html="masonToolbarIcons[button.icon]"
            />
            <span class="mason-tooltip">{{ getToolbarButtonTitle(button) }}</span>
          </button>
        </span>
      </div>
      <div class="mason-toolbar-side">
    ...
    <Editor
      ...
      @change="handleChange"
    />
    <div class="mason-custom-status" aria-live="polite">
    ...
    <div
      ...
      class="mason-table-dialog-container mason-autosave-dialog-container"
      role="dialog"
      aria-modal="true"
      aria-labelledby="mason-autosave-dialog-title"
      ...
      <div class="cs-dialog cs-autosave-dialog">
        <aside class="cs-autosave-sidebar">
          <div class="cs-dialog-header">
            <span id="mason-autosave-dialog-title">自动保存</span>
            <button
              type="button"
    ...
    <div
      v-if="linkDialogOpen"
      class="mason-table-dialog-container mason-simple-dialog-container"
    >
      <form class="cs-dialog" @submit.prevent="confirmLinkDialog">
        ...
        <div class="cs-dialog-item">
          <label class="cs-dialog-item-label" for="mason-link-url">
            链接地址
          </label>
          <input
            id="mason-link-url"
        ...
        <div class="cs-dialog-item">
          <label class="cs-dialog-item-label" for="mason-link-text">
            链接文字
          </label>
          <input
            id="mason-link-text"
    ...
    >
      <form class="cs-dialog" @submit.prevent="confirmImageDialog">
        ...
        <div class="cs-dialog-item">
          <label class="cs-dialog-item-label" for="mason-image-url">
            图片地址
          </label>
          <input
            id="mason-image-url"
        ...
        <div class="cs-dialog-item">
          <label class="cs-dialog-item-label" for="mason-image-alt">
            图片说明
          </label>
          <input
            id="mason-image-alt"
    ...
    <div
      v-if="codeDialogOpen"
      class="mason-table-dialog-container mason-code-dialog-container"
    >
      <div class="cs-dialog">
        <div>
          <div class="cs-dialog-header">
    ...
    <div
      ...
      class="mason-table-dialog-container"
