...
import { createMasonRemarkRehypeOptions } from "./remark-rehype-mason-handlers";
...
export function createMarkdownProcessorOptions(
  ...
  return {
    ...
    // Keep the rollback profile on ByteMD's native handlers. Mason compatibility
    // handlers are deliberately enabled only for the Mason profile.
    remarkRehype: profile === "mason-v1" ? createMasonRemarkRehypeOptions() : {},
  };
}
