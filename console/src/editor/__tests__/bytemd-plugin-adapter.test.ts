...
import { createBytemdEditorPlugins } from "../bytemd-plugin-adapter";
...
describe("ByteMD editor plugin composition", () => {
  it("keeps table editing when Vim is disabled", () => {
    const config = options();
    const result = createBytemdEditorPlugins(config);
  ...
  it("adds Vim without replacing table editing", () => {
    const config = options();
    const result = createBytemdEditorPlugins({ ...config, useVimKeymap: true });
