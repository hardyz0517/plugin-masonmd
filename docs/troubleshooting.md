...
当前插件会生成带 `bytemd-math-inline` 或 `bytemd-math-display` 类的 KaTeX 内容，并提供文章页样式。若同时使用 `plugin-katex`，不要将这两个类加入 plugin-katex 的处理选择器。
...
```text
/plugins/PluginBytemd/assets/ui/published/bytemd-markdown.css
```
...
4. 反向代理没有拦截 `/plugins/PluginBytemd/` 静态资源。
...
完整输入样例位于 [Mason Markdown 全语法测试文档](./compatibility/bytemd-all-syntax-test.md)。
...
插件文章样式使用 `bytemd-` 前缀，并由 Halo 页面头部处理器注入。若某个主题仍然显示异常，使用开发者工具检查主题是否覆盖了表格、代码块、列表、公式或 Mermaid 的通用选择器。
