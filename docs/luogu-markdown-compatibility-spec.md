# Luogu Markdown Compatibility Upgrade Specification

**Status:** Implemented; Halo integration matrix pending manual verification
**Scope:** ByteMD Console editor and Halo UC post editor
**Current baseline:** plugin-bytemd 1.10.56, Halo 2.25.0, ByteMD 1.22.x
**Reference:** GFM specification and the Luogu editor manual supplied with this task
...
- Markdown parsing, preview rendering, editor commands, and article persistence are coupled in bytemd.vue.
- Luogu-specific syntax is missing from the processor pipeline.
- The table dialog generates HTML for merged tables instead of Luogu-compatible Markdown source.
- Preview highlighting uses Highlight.js while Luogu uses PrismJS semantics.
...
- run post-sanitize Luogu rehype plugins only on already sanitized nodes;
...
2. Support all syntax described in the supplied Luogu editor manual, including:
   - callout directives;
   - nested directives;
   - table cell merging;
   - cute-table styles;
   ...
   - Luogu editor shortcuts and table editing behavior where feasible.
3. Produce the same semantic result in the editor preview, saved HTML, and published article.
4. Preserve raw Markdown source in Halo without replacing Luogu syntax with generated HTML.
...
- reproduce Luogu's private backend or private editor DOM;
...
- rewrite the editor only because Luogu uses CodeMirror 6.

## 3. Compatibility Contract

"Compatible with Luogu" is split into four independently testable contracts.

### 3.1 Source compatibility

The following Luogu source forms must be accepted without converting them to a private syntax:
...
Exact DOM class names used by Luogu are not required. Stable local semantic classes are preferred, provided the rendered result is equivalent and themeable.
...
- console/src/components/bytemd.vue
...
1. bytemd.vue directly owns too many unrelated responsibilities.
...
console/src/markdown/
  ...
  profiles.ts
  bytemd-processor.ts
  remark-luogu-directive.ts
  remark-luogu-table.ts
  remark-rehype-luogu-handlers.ts
  rehype-luogu-code.ts
  language-registry.ts
  sanitize-schema.ts
  fixtures/

console/src/editor/
  bytemd-plugin-adapter.ts
...
src/main/java/run/halo/bytemd/
  BytemdHeadProcessor.java
...
remarkParse
  -> registered remark plugins:
       ...
       remarkDirective
       remarkMath
       remarkLuoguNormalize
  -> remarkRehype(shared Luogu handlers)
  -> rehypeRaw
...
- luogu-v1 is the new target behavior.
- New syntax is enabled only in luogu-v1.
...
After the new pipeline has passed production validation, luogu-v1 becomes the default. Existing articles are not rewritten merely by installing the plugin; they are re-rendered only when saved or explicitly migrated.
...
Before implementation, compare the following against Luogu:
...
The result must be recorded in the luogu-v1 fixture. If Luogu uses hard breaks, retain remark-breaks in that profile. If Luogu follows soft-break behavior, remove it from luogu-v1 and keep it only in legacy if existing articles depend on it.
...
~~~html
<details class="luogu-callout luogu-callout-info" open>
  <summary>Title</summary>
  <div class="luogu-callout-body">...</div>
</details>
~~~

All four callout types use the same details/summary structure. The open attribute is emitted only for {open}; absence means the callout starts collapsed. The exact wrapper may change only if a Luogu fixture proves different behavior, and any replacement must preserve keyboard operation, open state, and nesting.
...
Alignment must not be applied to code blocks, tables, or list containers unless Luogu's actual behavior confirms it.
...
- tuack=N accepts one bounded positive column index according to Luogu fixtures;
- duplicate style directives are invalid and use deterministic fallback behavior.

### 6.5 Luogu table merge semantics
...
The mdast code node's lang and meta are not guaranteed to survive ByteMD's default code handler. The shared remarkRehype handler must explicitly copy validated source metadata to allowlisted intermediate properties before sanitization. rehypeLuoguCode then consumes those properties after sanitization and removes or normalizes them in the final output.

Required rules:

- an explicit language is passed to PrismJS;
- an absent language defaults to C++ for Luogu compatibility;
...
The output should have a stable structure similar to:

~~~html
<pre class="luogu-code-block" data-language="cpp">
  <code>
    <span class="luogu-code-line" data-line="5" data-highlighted="true">...</span>
  </code>
</pre>
~~~
...
Mermaid is an existing product extension, not GFM or Luogu syntax, but it must remain compatible throughout the migration.
...
Frontmatter is not part of the GFM or Luogu syntax contract. The current yaml-frontmatter CodeMirror mode must not be presented as proof that frontmatter is supported.
...
Maintain separate input and generated-HTML allowlists. The input sanitizer schema may allow only the intermediate properties required by the shared handlers, such as open, data-luogu-language, and data-luogu-meta. rowspan and colspan are generated by the trusted GFM table handler before sanitization and must be bounded integers. Post-sanitize output properties such as data-language, data-line, and data-highlighted must be created only by validated Luogu plugins and should not be added to the raw HTML input allowlist unless there is a documented compatibility reason. Generated values must come only from validated enums, booleans, and bounded integers.
...
The Luogu compatibility profile must cover this exact shortcut matrix, using Mod as Command on macOS and Ctrl elsewhere:
...
Introduce a LuoguTableModel that understands both ordinary GFM tables and merge markers.
...
- split merged cells;
- serialize back to Luogu Markdown;
...
The adapter may delegate low-level cursor movement to mte-kernel, but serialization, merge topology, malformed-input handling, and source round trips belong exclusively to LuoguTableModel.
...
Syntax compatibility and CodeMirror implementation parity are separate release dimensions. The Luogu manual also describes Ctrl+F search, code folding, and Alt-based multi-selection. These must be tracked explicitly:
...
- multi-selection is a separate capability and must not be claimed as Luogu parity until Chinese input and IME behavior are tested;
...
Semantic Markdown styles belong in luogu-markdown.scss, scoped to the editor preview and reusable article preview container.

The style layer must provide:

- light and dark tokens;
- callout colors and icons without excessive saturation;
- Luogu-like table styles;
...
All generated classes must use a stable prefix such as luogu- or hardy-markdown-. Avoid generic global selectors such as .table, .code, or .active.
...
The current BytemdHeadProcessor contains a large Mermaid CSS text block inside Java. Replace that implementation with a versioned link to a packaged static stylesheet. The stylesheet must contain only prefixed Luogu syntax, Mermaid, code-block, and KaTeX-support styles required by content generated by this plugin.
...
- Copy the Luogu manual examples into fixtures.
...
- Move processor construction out of bytemd.vue.
...
- Decide soft-break semantics from Luogu fixtures.
...
- Implement Luogu marker parsing and HAST transformation.
...
Exit criteria: code snapshots match the defined Luogu behavior for all supported languages and metadata cases.
...
- Compare legacy and luogu-v1 output for existing articles.
- Make luogu-v1 the default only after validation.
...
- New Luogu syntax may render as literal text under an older plugin, but must never corrupt raw source.
...
1. Every syntax example from the supplied Luogu manual has a fixture and passes.
...
5. The table dialog never emits HTML for a supported Luogu merge operation.
6. Invalid syntax produces a safe fallback and a diagnostic.
7. Raw HTML remains sanitized.
8. Post-sanitize Luogu plugins cannot inject arbitrary raw HTML or unvalidated attributes.
...
The following must be settled using Luogu behavior fixtures rather than assumptions:
...
- which subset of raw HTML Luogu accepts while Halo security still permits it.
