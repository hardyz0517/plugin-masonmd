# Luogu Markdown 全语法测试文档

这是一份用于测试 plugin-bytemd `luogu-v1` 配置的综合文档。正常语法应正确渲染；文末的安全边界和异常语法用于确认编辑器能够安全、稳定地降级。
...
下面的 `^` 表示向上合并，`<` 表示向左合并。`>` 不是洛谷文档定义的正常合并语法，异常表格只能局部降级为普通单元格，不能让整张表失效。检查预览、保存、刷新、重新编辑后表格拓扑是否保持不变。
